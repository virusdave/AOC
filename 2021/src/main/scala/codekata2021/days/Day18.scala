...
object Day18 extends ParserPuzzle {
  ...
  lazy val eNum: Parser[Element2]  = num ^^ Num.apply
  lazy val ePair: Parser[Element2] = pair
  lazy val pair: Parser[Pair]      = "[" ~> (eNum | ePair) ~ ("," ~> (eNum | ePair) <~ "]") ^^ { case l ~ r => Pair(l, r) }

  sealed trait Element2
  case class Num(v: Int) extends Element2 {
    override def toString: String = v.toString
  }

  final case class Pair(l: Element2, r: Element2) extends Element2 {
...
//            .debug
  ...
  override def part1: Option[Part] = new Part {
    override def solution: RIO[Any, Any] =
      inputs.map(parseAll(pair, _).get).reduceLeft(_ + _).magnitude
        .zio
  }.some
