...
object Day13 extends ParserPuzzle {
  ...
  val dot: Parser[Dot]     = num ~ ("," ~> num) ^^ { case l ~ r => Dot(l, r) }
  val foldx: Parser[FoldX] = "fold along x=" ~> num ^^ (FoldX)
  val foldy: Parser[FoldY] = "fold along y=" ~> num ^^ (FoldY)

  def foldDots(dots: Set[Dot], fold: Fold): Set[Dot] = fold match {
