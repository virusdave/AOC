...
object Day04 extends ParserPuzzle {
  ...
  case class Board(nums: Map[Int, (Set[(Int, Int)], Boolean)]) {
  ...
  val drawn: Parser[List[Int]] = rep1sep(num, ",")
  val board: Parser[Board] = repN(5, repN(5, num)) ^^ { ns =>
  ...
  val lines: Parser[(List[Int], List[Board])] = drawn ~ rep1(board) ^^ { case l ~ r => (l -> r) }
