...
object Day11 extends ParserPuzzle {
  ...
  private def step(in: Grid[Int], plus: Grid[Int], flashed: Set[(Int, Int)]): (Grid[Int], Set[(Int, Int)]) = {
  ...
  private def inputs = in2.linesIterator.toIndexedSeq.map(_.toIndexedSeq.map(_ - '0'))
